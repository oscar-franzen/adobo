...
def force_graph(obj, name=(), iterations=1000,
                edgeWeightInfluence=1.0, jitterTolerance=1.0,
                barnesHutOptimize=True, scalingRatio=2.0, gravity=1.0,
    ...
    edgeWeightInfluence : `float`
        How much influence to edge weights. 0 is no influence and 1 is
        normal.  Default: 1.0
    jitterTolerance : `float`
        Amount swing. Lower gives less speed and more
        precision. Default: 1.0
    barnesHutOptimize : `bool`
        Run Barnes Hut optimization. Default: True
    scalingRatio : `float`
        Amount of repulsion, higher values make a more sparse
        graph. Default: 2.0
    gravity : `float`
        Attracts nodes to the center. Prevents islands from drifting
        away. Default: 1.0
    ...
    forceatlas2 = ForceAtlas2(outboundAttractionDistribution=True,
                              ...
                              gravity=gravity, verbose=verbose)
    for l in targets:
        item = targets[l]
        if verbose:
...
def pca(obj, method='irlb', normalization=None, ncomp=75, genes='hvg',
    ...
    Notes
    -----
    Scaling of the data is achieved by setting scale=True (default),
    which will center (subtract the column mean) and scale columns
    ...
    genes : `{'hvg', 'all'}` or `list`
        If a string, the allowed values are 'hvg' to use only the
        highly variable genes or 'all' to use all genes. If a list,
        then the list specifies the list of genes to use. Default:
        'hvg'
    scale : `bool`
        Scales input data prior to PCA. Default: True
    use_combat : `bool`
    ...
    .. [2] Baglama et al (2005) Augmented Implicitly Restarted Lanczos
           Bidiagonalization Methods SIAM Journal on Scientific
           Computing
...
"hvg" (recommended) or "all".')
    ...
    for k in targets:
        ...
        if verbose:
            v = (method, k, '{:,}'.format(
                data.shape[0]), '{:,}'.format(data.shape[1]))
...
def tsne(obj, run_on_PCA=True, name=None, perplexity=30, n_iter=2000,
         seed=None, verbose=False, **args):
    """Projects data to a two dimensional space using the tSNE
    algorithm.

    Notes
    -----
    It is recommended to perform this function on data in PCA
    space. This function calls :py:func:`sklearn.manifold.TSNE`, and
    any additional parameters will be passed to it.
    ...
    obj : :class:`adobo.data.dataset`
          A dataset class object.
    run_on_PCA : `bool`
        To run tSNE on PCA components or not. If False then runs on
        the entire normalized gene expression matrix. Default: True
    name : `str`
        The name of the normalization to operate on. If this is empty
        or None then the function will be applied on all
        normalizations available.
    perplexity : `float`
        From [1]: The perplexity is related to the number of nearest
        neighbors that is used in other manifold learning
        algorithms. Larger datasets usually require a larger
        perplexity. Consider selecting a value between 5 and
        50. Different values can result in significanlty different
        results. Default: 30
    n_iter : `int`
    ...
    References
    ----------
    .. [1] van der Maaten, L.J.P.; Hinton, G.E. Visualizing
...
https://oscar-franzen.github.io/adobo/adobo.html#adobo.normalize.norm')
    ...
    if verbose and not run_on_PCA:
        warning('Running tSNE on the entire gene expression matrix is not \
recommended.')
    for k in targets:
        item = targets[k]
        if not run_on_PCA:
...
adobo.dr.pca()')
            X = item['dr']['pca']['comp']
        if verbose:
            print('Running tSNE (perplexity %s) on the %s normalization' %
                  (perplexity, k))
...
def umap(obj, run_on_PCA=True, name=None, n_neighbors=15,
         distance='euclidean', n_epochs=None, learning_rate=1.0,
         min_dist=0.1, spread=1.0, seed=None, verbose=False, **args):
    """Projects data to a low-dimensional space using the Uniform
    ...
    n_neighbors : `int`
        ...
        neighboring sample points) used for manifold
        approximation. Larger values result in more global views of
        the manifold, while smaller values result in more local data
        being preserved.  In general values should be in the range 2
        to 100. Default: 15
    distance : `str`
        The metric to use to compute distances in high dimensional
    ...
    n_epochs : `int`
        ...
        embeddings. If None is specified a value will be selected
        based on the size of the input dataset (200 for large
        datasets, 500 for small). Default: None
    learning_rate : `float`
        The initial learning rate for the embedding
        optimization. Default: 1.0
    min_dist : `float`
        The effective minimum distance between embedded
        points. Default: 0.1
    spread : `float`
...
https://oscar-franzen.github.io/adobo/adobo.html#adobo.normalize.norm')
    ...
    if name is None or name == '':
        ...
        targets[name] = obj.norm_data[name]
    if verbose and not run_on_PCA:
        warning('Running UMAP on the entire gene expression matrix is \
not recommended.')
    for k in targets:
        item = targets[k]
        if not run_on_PCA:
        ...
        reducer = um.UMAP(random_state=seed, verbose=verbose,
                          n_neighbors=n_neighbors, metric=distance,
                          n_epochs=n_epochs,
...
def jackstraw(obj, normalization=None, permutations=500, ncomp=None,
              subset_frac_genes=0.05, score_thr=1e-03, fdr=0.01,
              retx=True, verbose=False):
...
components computed by adobo.dr.pca(...)')
    ...
    for perm in np.arange(0, permutations):
        ...
        # permutate every row
        data_perm = [np.random.permutation(_col)
                     for g, _col in data_perm.iterrows()]
    ...
    n = [q1+q2 for q1, q2 in zip(['PC']*res.shape[1],
                                 res.columns.values.astype(str))]
    res.columns = n
...
def genes2scores(obj, normalization=None, genes=[], bins=25, ctrl=100,
                 retx=True, metadata=None):
    ...
    metadata : `str`
        If this is set to a string, then the scores will be set as a
        meta data variable with this column name. Default: None

    References
    ----------
    .. [1] Tirosh et al. (2016) Science. Dissecting the multicellular
    ...
    for g in genes:
        ret.append(
            binned[binned == binned[binned.index == g].values].sample(ctrl).index)
    con = []
    for g in ret:
...
def regress(obj, target_vars=[], normalization=None):
    ...
    Notes
    -----
    This function can be used to remove known confounding variables
    such as ambient gene expression modules, cell cycle genes or known
    experimental batches. It fits a linear model using numpy's least
    square method (numpy.linalg.lstsq), predicts expression values
    from the model and then extracts the residuals, which become the
